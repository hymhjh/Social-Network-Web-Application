...
<form method="post">
Search Diaries: <input type="text" name="act" value="" />
...
<?php
    ...
    $sql="select * from DIARY where title LIKE '%{$name}%' or text LIKE '%{$name}%'";
    $res=mysql_query($sql,$conn);
    if ($res !== false) {
        while($r = mysql_fetch_array($res, MYSQL_NUM)) {
            echo "<div style='text-align:center;'><td><a href='cons_diary.php?id=$r[0]&vn=$uname&un=$r[9]'>$r[1]</a></td></div>";
            echo"<br>";
        }
    }
    
    $query=mysql_query("call popular_dairy()",$conn);
    echo "<h5 style='font-size:18pt;color:#3b5999; text-shadow: 0 8px 9px #c4b59d, 0px -2px 1px #fff;text-align:center;'><p>POPULAR DIARIES LIST</p></h5>";
    if ($query !== false) {
        echo"<br>";
        while($row = mysql_fetch_array($query, MYSQL_NUM)) {
            echo "<div style='text-align:center;'><td><a href='cons_diary.php?id=$row[0]&vn=$uname&un=$row[1]'>$row[2]</a></td></div>";
            echo"<br>";
        }
    }
    ?>
